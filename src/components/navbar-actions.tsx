...
import { Bell, Heart, LogOut, User as UserIcon, Settings, UserPlus, Lightbulb as BulbIcon } from "lucide-react";
  ...
  return (
    <div className="flex items-center gap-3 md:gap-4 pr-4">
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-[300px] font-tajawal p-2 rounded-2xl border-border shadow-2xl bg-card max-h-[400px] overflow-y-auto custom-scrollbar">
                ...
                <DropdownMenuItem key={n.id} className={cn(
                  ...
                  <div className="flex flex-col gap-0.5">
                    <span className="text-[13px] font-bold leading-tight">
                      {n.type === 'helpful' ? `${data.fromUser} استفاد من منشورك` : `${data.fromUser} بدأ بمتابعتك`}
                    </span>
      ...
      <DropdownMenu>
        <DropdownMenuTrigger className="relative h-10 w-10 rounded-full overflow-hidden border-2 border-border hover:border-primary transition-colors bg-gray-100 dark:bg-gray-800 flex items-center justify-center font-tajawal cursor-pointer group shadow-sm focus:outline-none">
          {avatarUrl ? (
            <NextImage 
              src={avatarUrl} 
              alt={userName} 
        ...
        <DropdownMenuContent align="end" className="w-64 font-tajawal p-2 rounded-2xl border-border shadow-xl bg-card">
          <div className="p-3">
             <div className="flex flex-col gap-0.5">
          ...
          <DropdownMenuGroup className="p-1">
            <DropdownMenuItem 
              ...
              className="flex items-center gap-3 p-3 rounded-xl cursor-pointer hover:bg-white/10 text-white transition-colors focus:bg-white/10"
            >
              <Settings className="w-4 h-4" />
              <span className="text-sm font-bold">الإعدادات والملف الشخصي</span>
            </DropdownMenuItem>
            <DropdownMenuItem 
